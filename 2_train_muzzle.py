...
for filename in os.listdir('F:\\xampp\\htdocs\\training_cow\\muzzle_additional2'):
    idx1 = filename.find('_')
    idx2 = filename.find(')')
    st = filename[idx1+1:idx2+1]
    img = Image.open(os.path.join('F:\\xampp\\htdocs\\training_cow\\muzzle_additional2',filename))
    x_train.append(np.array(img))
    y_train.append(data[st])
    training_samples = training_samples + 1

for filename in os.listdir('F:\\xampp\\htdocs\\training_cow\\muzzle_additional3'):
    idx1 = filename.find('_')
    idx2 = filename.find(')')
    st = filename[idx1+1:idx2+1]
    img = Image.open(os.path.join('F:\\xampp\\htdocs\\training_cow\\muzzle_additional3',filename))
    x_train.append(np.array(img))
    y_train.append(data[st])
    training_samples = training_samples + 1
